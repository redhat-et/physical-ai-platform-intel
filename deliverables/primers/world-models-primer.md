...
| Horizon        | Examples                                                                                                                            |
| -------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| Deployed today | Tesla FSD (millions of vehicles), BADAS 2.0/Nexar, Cosmos (2M+ DLs), Sereact Cortex 2.0, Skild Brain (Foxconn/NVIDIA GPU assembly)  |
| Deploying 2026 | Wayve Tokyo robotaxi, GXO humanoid warehouse pilots, Alibaba Qwen-Robot (pilot), NVIDIA Isaac GR00T Reference Robot (academia)      |
...
**Funding and competitive signals indicate enterprise confidence:** AMI Labs raised $1.03B (JEPA commercialization), Skild AI $1.4B at $14B valuation (omni-bodied robot brain, now deployed on Foxconn lines), Physical Intelligence reportedly seeking $1B (π0.7 zero-shot generalization), World Labs $1B (3D world generation), Odyssey $310M (multi-agent world simulation), Periodic Labs $300M (materials discovery), Genesis AI $105M (full-stack dexterous robotics). Global robotics VC reached $40.7B in 2025 (9% of all venture funding). VLA research submissions to ICLR grew 18x in one year (9 at ICLR 2025 to 164 at ICLR 2026). The question is no longer "will world models work?" but "which approaches scale fastest and where?"
...
**Who's deploying what today:** VLAs dominate production robotics, and humanoid deployments are hitting industrial scale. Figure AI's BotQ factory produces 1 robot/hour (55+ Figure 03 units/week, 12K annual capacity); its retired F.02 loaded 90K+ sheet metal parts at BMW Spartanburg with 5mm precision. Unitree has shipped 5,500+ G1/H2 units and is filing a $610M Shanghai IPO. 1X is shipping 20K pre-ordered NEO humanoids ($20K each) and launched a dedicated World Model Lab (June 2026) to move beyond VLA-only control. These systems use VLA-style architectures based on GR00T N1 or Physical Intelligence's π0. Genesis AI takes a different approach with GENE-26.5: flow-matching across five modalities, demonstrating complex dexterous tasks like in-air Rubik's Cube solving with <1 hour of task-specific data, though no independent benchmarks have been published.

WAMs now have two industrial deployments: Sereact's Cortex 2.0 (manipulation planning) and Skild AI's Brain on Foxconn assembly lines building NVIDIA Blackwell GPU servers in Houston — the first large-scale factory deployment of a robot foundation model, accelerating Skild's data flywheel to $30M revenue in its first months. NVIDIA's GR00T N2 (planned late 2026) will integrate Cosmos as a world model backbone; the new Isaac GR00T Reference Humanoid Robot (Unitree H2 Plus + Jetson AGX Thor) gives academic partners a full-stack open reference design. JEPA-based approaches have exactly one confirmed production deployment (BADAS 2.0), making them the most theoretically promising but least commercially validated path.
