...
| Physics-informed WM | States with physics constraints | PhysicsNeMo, Emmi AI, PhysicsX | ✓ Yes |
| World Action Models | States + actions jointly | DreamZero, Being-H0.7, LingBot-VA 2.0 | ✓ Yes |
...
| Omnimodal WM | States + video + audio + actions | Cosmos 3, Wujie Physis-v0.1 (BAAI) | ✓ Yes |
...
| Agentic "world models" | "World Model" for digital agents | Most are narrow UI-transition simulators that memorize screen-state sequences rather than learning dynamics — they fail the transferability indicator. However, the four defining characteristics are domain-agnostic: systems that learn infrastructure or network dynamics from data, maintain state, and run action-conditioned what-if simulations (e.g., cybersecurity attack-path prediction, telecom KPI trajectory forecasting) can meet the definition even outside physical domains. Wang et al. (2025) propose a "levels × laws" taxonomy spanning physical, digital, social, and scientific domains. The litmus test applies regardless of domain: does it predict what happens next without executing the action? |
...
**The broader pattern:** As world models gain attention, expect more systems to adopt the label, not only in Physical AI but across cybersecurity (learned infrastructure models for attack-path simulation), telecommunications (action-conditioned network state prediction), and other domains with complex dynamics. The four defining characteristics provide a domain-agnostic filter: if it doesn't predict future states from learned representations, it may be valuable but isn't a world model. The defining question isn't whether the domain is physical, but whether the system learns, maintains state, predicts, and simulates.
...
**WAMs: Designed for L3-L4.** World Action Models represent the first architecture explicitly designed to reach L3-L4 in real-world robotics. They unify world modeling and action generation, enabling a model to simulate multiple action trajectories ("what if I grasp here vs. there?") and select the best one before execution. DreamZero reached 1,763 Elo on RoboArena trained only on DROID (now surpassed by Spirit v1.6 at 1,924 Elo and Cosmos3-Nano-Policy at 1,881), and Sereact's Cortex 2.0 is the first confirmed industrial WAM deployment (200+ systems, 1B+ production picks). NVIDIA's GR00T N2 (previewed at GTC Taipei June 2026, available late 2026) bets on this architecture, integrating Cosmos as the world model backbone. See Sidebar B for architecture details, cost analysis, and deployment status.
...
**The bottleneck may not be model scale.** A growing counternarrative (Karcini et al. 2026) argues that generalist robot intelligence is not primarily a scaling problem. The real gap is four missing *interfaces* for converting the world's abundant unstructured behavioral data (human motion, internet video, simulation rollouts) into grounded robot supervision: data interfaces for autolabelling, embodiment interfaces for cross-body retargeting, world-model interfaces for physics-grounded 3D reasoning, and reward interfaces for inferring task progress from video. Without these, simply training larger VLAs or WAMs hits diminishing returns. This shifts the platform investment thesis: infrastructure for data curation and embodiment transfer may matter more than raw model training compute.

**The benchmark crisis:** Popular robot manipulation benchmarks are less informative than they appear. A systematic audit (Jiang et al. 2026) found that LIBERO, the most-cited benchmark, is solvable by a 0.09B probe with no language encoder, scoring at or near reported SOTA. Models claiming 98% success collapse to 0-40% under moderate perturbations (changed instructions, object poses, viewpoints). CALVIN is similarly saturated. The implication: benchmark scores in recent VLA papers often reflect overfitting to narrow evaluation setups, not genuine manipulation capability. When evaluating vendor claims, ask for results on multiple benchmarks (RoboVerse, RoboChallenge Table30) and real-world validation (RoboArena), not just LIBERO numbers. For world models specifically, WorldOlympiad (Zhao et al. 2026) introduces a tripartite evaluation — physics faithfulness, geometric consistency, and interaction fidelity — that tests whether generated videos obey physical rules rather than just looking plausible. State-of-the-art models show substantial gaps in all three dimensions.
...
A related approach sidesteps weight updates entirely. Mem-World (Zhou et al. 2026) introduces surfel-indexed visual memory (W-VMem): rather than updating model weights with new observations, it maintains an explicit 3D memory structure that persists across manipulation steps, enabling the model to reason about occluded objects and track state changes without retraining. This doesn't solve continual learning — the underlying world model remains frozen — but it demonstrates that external memory architectures can partially bridge the gap between static models and adaptive agents. The pattern (frozen model + persistent external state) may prove more practical than weight-level adaptation for near-term deployments.
...
Models trained purely in simulation often fail when deployed in the real world. Friction, lighting, and sensor noise differ from their mathematical abstractions. Three strategies are emerging:

1. **Domain bridging** (established): Pretraining on real-world video before simulation fine-tuning (JEPA), domain randomization, or explicit style transfer (Cosmos Transfer). The goal is learning representations that generalize beyond any single environment's quirks.
2. **Co-training** (new): HyperSim (Dong et al. 2026) jointly trains on simulated and real data with adversarial trajectory generation, achieving 95% sim-to-real success with π0 across 400 real-world manipulation trials. The co-training enforces domain-invariant representations, and adversarial trajectories add 35% robustness under physical perturbations. This is the first framework to demonstrate production-viable sim-to-real transfer rates.
3. **Gap elimination** (paradigm shift): Real-is-Sim (Abou-Chakra et al. 2025) inverts the entire paradigm. Instead of training in simulation and transferring to reality, it builds a dynamic digital twin (via Gaussian splatting) that synchronizes with the real world at 60Hz. Policies always run on the simulated robot; the physical robot follows the twin's joint states. There is no transfer gap because simulation *is* the execution environment. Early results show virtual evaluations consistent with real-world outcomes, though scaling beyond single-task demonstrations is unproven.
...
| ------------------------ | ----------------- | ------------------------------------- | ------------------------------------------------------------------------------ |
| Real-time robot control  | Speed (<100ms)    | VLA, distilled WM, or sparse-MoE WAM  | Latency dominates; sparse-MoE WAMs (LingBot-VA 2.0) claim parity, unverified   |
...
| WAMs               | 7.4x training cost over VLAs; first industrial deploy (Sereact); latency gap narrowing via Fast-WAM (rep-only, sub-200ms) and LingBot-VA 2.0 (sparse MoE, claims 150 Hz, unverified)                               |
...
| Horizon | Examples |
| --- | --- |
| Deployed today | Tesla FSD v14 (millions of vehicles), BADAS 2.0/Nexar (2M clips, #1 all benchmarks), Cosmos 3 (open-weight), Sereact Cortex 2.0 (200+ systems, 1B+ picks), Skild Brain (Foxconn + ABB/UR partnerships) |
| Deploying 2026 | Wayve Tokyo robotaxi (Nissan/Uber), GXO humanoid warehouse pilots, Alibaba Qwen-Robot (pilot), NVIDIA Isaac GR00T Reference Robot (academia, Oct 2026), GR00T N2 (previewed GTC Taipei) |
| 1-2 years | AMI Labs commercial products (~1 year out per CEO) |
| Research | Scaling laws for world models, long-horizon temporal consistency, multi-agent world models |

**Funding and competitive signals indicate enterprise confidence:** Figure AI raised $1B at $39.5B (humanoid manufacturing at scale), AMI Labs $1.03B (JEPA commercialization), Skild AI $1.4B at $14B (omni-bodied robot brain, Foxconn + Zebra acquisition), NEURA Robotics up to $1.4B at ~$7B (NVIDIA/Amazon/Qualcomm), Physical Intelligence seeking $1B at $11B (π0.7 compositional skill transfer), World Labs $1B (3D world generation), AI2 Robotics $735M at $2.8B, Odyssey $310M (multi-agent world simulation), Generalist AI $400M (NVIDIA/Bezos), Mind Robotics >$1B at >$3B (Rivian spinout, EV production lines as live training environments), General Intuition $454M at $2.3B (world models trained on Medal gaming clip data). Agility Robotics is going public via SPAC merger with Churchill Capital Corp XI at $2.5B ($620M proceeds, $300M+ booked revenue) — the first pure-play humanoid to reach public markets. Global robotics VC reached $55.8B YTD 2026, already 2x the prior record ($40.7B in 2025). VLA research submissions to ICLR grew 18x in one year (9 at ICLR 2025 to 164 at ICLR 2026). The question is no longer "will world models work?" but "which approaches scale fastest and where?"
...
**The regulatory angle (timing matters):** FDA issued its first guidance on using AI in drug development in January 2025, signaling regulatory acceptance of AI-driven discovery but leaving validation pathways undefined. The EU AI Act's high-risk provisions for medical devices were originally scheduled for August 2026, but the Digital Omnibus agreement (May 2026) postpones medical device provisions to August 2028, giving organizations more runway but also reducing urgency. Noise filtering is both a technical and compliance advantage when models must demonstrate robustness to real-world data variability.
...
| WAM | ~150-800ms | Good (video backbone) | Industrial (Sereact, Skild), previewed (GR00T N2) |
| WAM (sparse MoE) | ~7ms (claimed) | Good (video backbone) | Open-weight (LingBot-VA 2.0) — 150 Hz claimed, pending verification |
...
**World Action Models (WAMs)** like DreamZero, Being-H0.7, Spirit v1.6 (current RoboArena #1 at 1,924 Elo), and GR00T N2 (previewed June 2026) unify state prediction and action generation in a single architecture, typically using video foundation models as backbones. They learn joint representations of observations and actions, enabling them to simulate future states *while* predicting optimal actions. Standard WAM inference runs at 590-800ms per action chunk (3-4x slower than VLAs) because the model must generate video alongside actions. The latency gap is narrowing from two directions. Fast-WAM demonstrates a representation-only mode that skips video generation at inference, cutting latency to sub-200ms while matching performance on simulation benchmarks, pending real-robot validation. Robbyant's LingBot-VA 2.0 (July 2026) takes a different approach: a sparse MoE backbone (~5B params) pretrained natively for causal embodied control, claiming 150 Hz real-time inference on a single GPU — which, if verified, would make it *faster* than VLAs. The claim is plausible (sparse MoE reduces active parameters per forward pass, and asynchronous foresight inference overlaps prediction with execution) but awaits independent benchmarking. Beyond latency, deployment constraints are receiving attention: Kairos (Wang et al. 2026) is the first WAM stack to treat memory footprint, hardware compatibility, and inference budget as first-order architectural constraints rather than afterthoughts, producing deployment-aware action plans that respect device-specific resource limits — addressing the gap between research performance on high-end GPUs and edge deployment feasibility. Sereact's Cortex 2.0 is the first confirmed industrial WAM deployment, using latent-space trajectory scoring for manipulation planning.
...
**Who's deploying what today:** VLAs dominate production robotics, and humanoid deployments are hitting industrial scale. Figure AI's BotQ factory produces 1 robot/hour (350+ Figure 03 units produced, 80% first-pass yield, 12K annual capacity); its retired F.02 loaded 90K+ sheet metal parts at BMW Spartanburg with 5mm precision, and 40 Figure 03 units now operate across four BMW plants (Spartanburg, Leipzig, Munich, Regensburg) at $25/robot-hour RaaS pricing. Figure raised $1B Series C at $39.5B valuation. Unitree cleared its STAR Market IPO listing review (seeking $620M at ~$6.2B valuation), reported ¥1.7B revenue in 2025 (+335% YoY, first profitable year), and its H2 Plus was selected as the NVIDIA GR00T reference humanoid. 1X has 10K+ NEO humanoid deposits ($20K each, US delivery Q3-Q4 2026) and launched a dedicated World Model Lab (June 2026, hired Sam Sinha ex-Luma AI) to move beyond VLA-only control. These systems use VLA-style architectures based on GR00T N1 or Physical Intelligence's π0 (π0.7 unveiled April 2026 with compositional skill transfer). Genesis AI takes a different approach with GENE-26.5: flow-matching across five modalities, demonstrating complex dexterous tasks like in-air Rubik's Cube solving with <1 hour of task-specific data, though no independent benchmarks have been published.

WAMs now have two industrial deployments: Sereact's Cortex 2.0 ($110M Series B, 200+ systems deployed, 1B+ production picks, 1 intervention per 53K picks, expanding to US) and Skild AI's Brain on Foxconn assembly lines building NVIDIA Blackwell GPU servers in Houston — the first large-scale factory deployment of a robot foundation model, with $30M revenue and expanded partnerships (ABB, Universal Robots, MiR, Zebra robotics division acquisition). NVIDIA's GR00T N2 (previewed at GTC Taipei June 2026, based on DreamZero WAM architecture, available late 2026) integrates Cosmos as the world model backbone; the new Isaac GR00T Reference Humanoid Robot (Unitree H2 Plus + Sharpa hands + Jetson Thor) gives academic partners (Ai2, ETH Zurich, Stanford, UCSD) a full-stack open reference design. On the open-weight side, Robbyant's LingBot-VA 2.0 (July 2026, Apache-2.0, ~5B params) is the first WAM to claim real-time inference parity with VLAs, achieving 92.9%/91.6% on RoboTwin 2.0 Easy/Hard and 98.5% on LIBERO; it completes a six-model open embodied AI stack from Ant Group. JEPA-based approaches have exactly one confirmed production deployment (BADAS 2.0: trained on 2M clips, 99.4% AP, #1 on all four major benchmarks, Flash Lite runs 5.9ms on Jetson Thor, serving Waymo/Lyft/IBM/NVIDIA), making them the most theoretically promising but least commercially validated path.
...
**NVIDIA's positioning:** Strong in both VLA (GR00T N1 deployed broadly) and WAM (GR00T N2 previewed at GTC Taipei June 2026, based on DreamZero architecture, available late 2026). Primary VLA competitors are Physical Intelligence's π0 (π0.7 showing compositional skill transfer) and Google DeepMind's Gemini Robotics (~250ms latency, cloud-first architecture). Spirit AI's Spirit v1.6 (1,924 Elo, current RoboArena #1) is an emerging WAM competitor. The real test comes in late 2026 when GR00T N2 deploys: if WAMs prove faster and more capable than distilled VLAs, NVIDIA pulls ahead. If the distillation pattern continues to dominate, the market fragments across multiple vendors.
...
2. **Physics-informed models** blend learned dynamics with explicit constraints (partial differential equations, conservation laws, kinematic limits). Five architecture families have emerged: Fourier Neural Operators (FNO) for regular grids, DeepONet for irregular domains, Graph Neural Operators for complex meshes, Physics-Informed Neural Networks (PINNs) for sparse-data regimes, and Hamiltonian Neural Networks for energy-conserving dynamics. NVIDIA's PhysicsNeMo (Apache 2.0) supports multiple operator architectures, achieving 500x speedup over traditional CFD while guaranteeing mass and momentum conservation. The competitive landscape has expanded: PhysicsX ($2.4B valuation, GM and JLR production deployments) and Mistral's Emmi AI (~€300M acquisition, proprietary surrogates) compete on the platform side, while open-source frameworks like DeepXDE (MIT license, 4.3K GitHub stars, 5-backend portability) and NeuralOperator (PyTorch Ecosystem) provide hardware-portable alternatives. These approaches need less data than pure learning because physics priors constrain the hypothesis space, but architecture selection is critical — see noise sensitivity below.

   **Architecture selection matters for noisy data.** A critical finding for Physical AI applications: Fourier Neural Operators (FNO) suffer 10,000x error degradation with just 0.1% input noise, while DeepONet remains robust under the same conditions (Lu et al. 2022). This parallels the noise-filtering advantage of latent-space prediction in medical imaging (see Sidebar A): architecture choice determines whether a model amplifies or filters real-world sensor noise. For edge deployments with noisy sensor data — the typical Physical AI scenario — this makes DeepONet-family architectures strongly preferable to FNO for production surrogates, despite FNO's superior performance on clean benchmarks.
...
**NVIDIA's positioning varies by domain.** Strong in **product design**: PhysicsNeMo (Apache 2.0) integrates with Omniverse for digital twins in automotive, aerospace, and energy, though it now faces competition from PhysicsX (production deployments at GM and JLR, $489M funding from Siemens and NVIDIA themselves) and Mistral's Emmi AI (proprietary, acquired for ~€300M). Weak in **drug discovery** and **materials**: no competitive offerings against Insilico Medicine (ISM001-055 now in Phase 3), Recursion Pharmaceuticals, or Periodic Labs. NVIDIA provides the GPUs and infrastructure — and notably invested in PhysicsX, hedging against PhysicsNeMo's open-source model — but domain expertise and specialized world models come from vertical-focused startups. Foundation world models generalize across physics-heavy domains (Cosmos for video, GR00T for robotics), but scientific discovery requires domain-specific architectures where NVIDIA's ecosystem competes rather than dominates.
...
- **Tesla Full Self-Driving v14** — Occupancy networks deployed in millions of vehicles globally, representing the largest real-world deployment of world model concepts for autonomous driving decision-making. At CVPR 2026, Tesla revealed a unified foundational model architecture shared across FSD, Optimus humanoid, and Digital Optimus simulation, with a "Ground Truth Heuristic Engine" for synthetic training scenario generation.
- **Nexar BADAS 2.0** — First production deployment of V-JEPA2 for driver-assistance and collision avoidance. Trained on 2M clips, 99.4% AP, #1 on all four major benchmarks. Three model sizes (300M/86M/22M params); Flash Lite runs 5.9ms on Jetson Thor. Serves Waymo, Lyft, IBM, NVIDIA. Outperforms fine-tuned Cosmos at 91x smaller size.
...
- **[WorldOlympiad](https://arxiv.org/abs/2606.11129)** (Zhao et al. 2026) — Tripartite benchmark for video-based world models evaluating physics faithfulness, geometric consistency, and interaction fidelity across gaming, robotics, and real-world scenarios.
...
- **[LingBot-VA 2.0](https://arxiv.org/abs/2607.08639)** (Zhang et al. 2026) — Native causal video-action foundation model from Robbyant/Ant Group; sparse MoE backbone (~5B params), semantic visual-action tokenizer, claims 150 Hz inference; 92.9%/91.6% RoboTwin 2.0, 98.5% LIBERO; Apache-2.0.

**Sim-to-Real & Deployment:**

- **[HyperSim](https://arxiv.org/abs/2605.26638)** (Dong et al. 2026) — Holistic sim-to-real framework achieving 95% success with π0 across 400 real-world manipulation trials via co-training and adversarial trajectory generation.
- **[Real-is-Sim](https://arxiv.org/abs/2504.03597)** (Abou-Chakra et al. 2025) — Dynamic digital twin via Gaussian splatting synchronizing at 60Hz; policies run on simulation, physical robot follows; eliminates sim-to-real transfer gap.
- **[Robots Need More Than VLAs & World Models](https://arxiv.org/abs/2606.06556)** (Karcini et al. 2026) — Position paper identifying four missing interface types (data, embodiment, world-model, reward) as the real bottleneck for general-purpose robotics, not model scale.

**Industry Analysis:**

- **Insilico Medicine ISM001-055 (rentosertib)** — First AI-designed drug to reach Phase 3 trials (initiated July 2026; 320 patients, 47 centers, 52-week trial), compressing drug development timeline from 4-5 years to 18 months through learned world models.
